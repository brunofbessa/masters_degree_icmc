...
class UPBG(BaseEstimator, ClassifierMixin):
    ...
    def __init__(self, n_components, alpha=0.05, beta=0.0001, local_max_itr=50,
                 ...
                 save_interval=-1, is_semisupervised=True, feature_names=None, disable_tqdm=True):
    ...
    def global_propag(self):
        try:
            for i in tqdm(range(self.nwords), ascii=True, desc='global propagation:   ', disable=self.disable_tqdm):
    ...
    def bgp(self, labelled=None):
        try:
            ...
            while global_niter < self.global_max_itr:            
                for j in tqdm(range(self.ndocs), ascii=True, desc=f'docs processed (itr {global_niter})', disable=self.disable_tqdm):
                    self.local_propag(j)
                    #if not self.unlabeled[j]:
                    #    self.supress3(j)
                self.global_propag() 
